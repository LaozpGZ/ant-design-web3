...
declare const window: OKXAptosWindow;
...
export class OKXWalletAdapter extends BaseWalletAdapter {
  ...
  async connect(): Promise<void> {
    try {
      ...
      this._network = await window.okxwallet!.aptos!.network();
  ...
  async signAndSubmitTransaction(
    ...
    try {
      if (!this.connected || !window.okxwallet?.aptos) {
        throw new Error('Wallet not connected');
      }

      const result = await window.okxwallet.aptos.signAndSubmitTransaction(input.data);
  ...
  async signTransaction(
    ...
    try {
      ...
      const result = await window.okxwallet.aptos.signTransaction(input.transaction);
